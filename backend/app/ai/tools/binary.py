...
async def _handle_search_binary_content(input: dict, context: ToolContext) -> str:
    ...
    try:
        functions = await ghidra_service.get_functions_if_cached(
            path, context.firmware_id, context.db,
        )
        for fn in functions:
...
async def _handle_start_binary_analysis(
    ...
    await ghidra_service.mark_run_started(
...
async def _handle_start_function_decompile(
    ...
    cached = await ghidra_service._get_cached(
    ...
    status_row = await ghidra_service.get_function_run_status(
    ...
    await ghidra_service.mark_function_run_started(
