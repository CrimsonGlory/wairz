...
async def _post_process_pipeline(
    ...
    *,
    ...
    if is_tar and firmware_dir and storage_path:
        try:
            if tarfile.is_tarfile(storage_path):
                ...
                # vendor wrapper around nested archives — the DS1
                # d360f8f5 case (live_base.tar.gz containing 2 inner
                ...
                if fs_root:
                    ...
                    if is_dense:
                        ...
                        # recursion exposed a real rootfs (DS1 Moto
    ...
    if update_stage:
        ...
        # in WALKER_AUTO_TRIGGERS (currently 33 safe-runners post the C2
        # module-reachability walker 2026-05-29 — count is authoritative
        # via len(get_walker_auto_triggers()); this docstring is
        # informational only, Rule #21 docstring-sync).
        #
