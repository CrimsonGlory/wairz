...
class FindingService:
    ...
    async def list_by_project(
        ...
        if firmware_id:
            # Prefer the multi-version association (firmware_versions) when
            # present; also match the legacy single firmware_id column so
            # pre-migration rows remain filterable.
            stmt = stmt.where(
